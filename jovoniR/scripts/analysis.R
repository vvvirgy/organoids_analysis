...
RNA_PATH = "/orfeo/cephfs/scratch/cdslab/gsantacatterina/organoids_analysis/jovoniR/results/RNA/lfc_res_clean.rds"
...
df %>% 
  ggplot(mapping = aes(x = karyotype, y = lfc, colour = omic)) +
  geom_violin() +
  theme_bw()
...
df <- df %>%
  left_join(noise_model, by = "omic") %>%
  mutate(
    z = (lfc - mu) / sigma,
    p_one_sided = pnorm(-abs(z)),
    lfc_signif  = p_one_sided <= ALPHA
  ) %>% 
  dplyr::select(!c(z, p_one_sided))

df_volcano_and_bar = df %>% 
  dplyr::select(pval, lfc, name, karyotype, omic, lfc_signif) %>% 
  dplyr::group_by(omic, karyotype) %>% 
  dplyr::mutate(adj_pval = p.adjust(pval, "BH") + 1e-300) %>% 
  dplyr::mutate(signif = ifelse(adj_pval <= ALPHA, "< 0.05", "ns")) %>% 
  dplyr::mutate(FC_class = ifelse((adj_pval <= ALPHA) & (lfc > 0) & lfc_signif, "Up", ifelse((adj_pval <= ALPHA) & (lfc < -0) & lfc_signif, "Down", "Not differential"))) %>% 
...
df_classified <- df %>%
  ...
  dplyr::group_by(omic, karyotype) %>%
  dplyr::mutate(
    CS_class = ifelse(CS > 0 & p_cs <= ALPHA, "CS>0", "CS<=0")
  ) %>%
  dplyr::ungroup() %>%
  dplyr::select(-mu, -sigma, -p_cs)

df_classified %>% 
  dplyr::group_by(karyotype, omic) %>% 
  dplyr::count(CS_class)
