...
df_DNA_path = "/orfeo/cephfs/scratch/cdslab/vgazziero/organoids_prj/data/processed_data/dna/cumulative_dp.rds"
df_DNA = readRDS(df_DNA_path)

genes = df_DNA %>% 
  dplyr::filter(!chr %in% c("chrX", "chY")) %>% 
...
for (g in genes) {
  ...
  if (length(unique(karyotypes)) == 1) next
  ...
  sample_ids = clusters
  suppressMessages({fit = devil::fit_devil(input_matrix = t(as.matrix(counts)), design_matrix = design_matrix, clusters = sample_ids,
  ...
  for (c in coeffs) {
    ...
    test_res = devil::test_de(devil.fit = fit, contrast = contrast_vec) %>% dplyr::mutate(name = g)
...
saveRDS(DNA_res, "/orfeo/cephfs/scratch/cdslab/gsantacatterina/organoids_analysis/jovoniR/results/DNA_lfc.rds")
