...
rna = readRDS('data/transcriptomics_data_all_genes_v5.rds') %>% 
...
rna = rna %>% 
  tidyr::separate(col = Polyphen_class, sep = '\\(', into = c('class', 'score')) %>%
  mutate(score = as.numeric(gsub('\\)', '', score)))

# consider synonimous mutations as wild-type
prot = readRDS('data/proteogenomics_data_all_genes_new_norm_v4.rds') %>%
  dplyr::mutate(is_mutated = ifelse(mut_consequence == 'synonymous_variant', FALSE, is_mutated)) %>% 
  dplyr::mutate(mutation_status = ifelse(is_mutated == TRUE, 'Mutated', 'Wild-type')) %>%
  dplyr::mutate(mutation_status = factor(mutation_status, levels = c('Wild-type', 'Mutated'))) %>%
...
saveRDS(test_expr_rna, 'data/test_expr_rna_bootstrap_v4.rds')
saveRDS(test_expr_prot, 'data/test_expr_prot_bootstrap_v4.rds')