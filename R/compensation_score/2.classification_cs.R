...
df_mean = readRDS(paste0('data/compensation/cs_mean_across_karyos_min_', min_samples,'.rds')) #%>% 
...
df_groups <- df_mean %>%
  ...
  mutate(reg_group = case_when(
    ...
    mean_CS_RNA > rna_cuts[2] & mean_CS_protein < prot_cuts[1] ~ "(RNA-heavy)",
    mean_CS_RNA > rna_cuts[2] & mean_CS_protein > prot_cuts[2] ~ "(RNA-prot heavy)",
    
    # Group 2: Low RNA CS (<35th) and High Protein CS (>65th)
    mean_CS_RNA < rna_cuts[1] & mean_CS_protein > prot_cuts[2] ~ "(Prot-heavy)",
    mean_CS_RNA < rna_cuts[1] & mean_CS_protein < prot_cuts[1] ~ "(RNA-Prot light)",
