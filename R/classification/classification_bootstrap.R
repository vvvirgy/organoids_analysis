...
rna = readRDS('data/transcriptomics_data_all_genes_v3.rds') %>% 
...
prot = readRDS('data/proteogenomics_data_all_genes_new_norm_v2.rds') %>%
  dplyr::mutate(mutation_status = ifelse(is_mutated == TRUE, 'Mutated', 'Wild-type')) %>%
  dplyr::mutate(mutation_status = factor(mutation_status, levels = c('Wild-type', 'Mutated'))) %>%
...
saveRDS(test_expr_rna, 'data/test_expr_rna_bootstrap_v2.rds')
saveRDS(test_expr_prot, 'data/test_expr_prot_bootstrap_v2.rds')